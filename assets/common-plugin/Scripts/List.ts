...
export default class List extends cc.Component {
    ...
    _onScrolling(ev: cc.Event = null) {
        ...
        if (this.cyclic) {
            ...
            let add: any = this._sizeType ? CompatibleTool.position(0, addVal) : CompatibleTool.position(addVal, 0);
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            ...
            item.setPosition(CompatibleTool.position(data.x, data.y));
        ...
        listItem.showAni(aniType, () => {
            ...
            for (let n: number = newId != null ? newId : curLastId; n >= listId + 1; n--) {
                ...
                if (item) {
                    ...
                    acts = [
                        cc.moveTo(sec, CompatibleTool.position(posData.x, posData.y)),
                    ];
                    if (n <= listId + 1) {
                        haveCB = true;
        ...
        switch (t._alignCalcType) {
            case 1://单行HORIZONTAL（LEFT_TO_RIGHT）、网格VERTICAL（LEFT_TO_RIGHT）
                ...
                if (offset != null)
                    ...
                    targetX -= t._leftGap;
                pos = CompatibleTool.position(targetX, 0);
                break;
            case 2://单行HORIZONTAL（RIGHT_TO_LEFT）、网格VERTICAL（RIGHT_TO_LEFT）
                ...
                if (offset != null)
                    ...
                    targetX += t._rightGap;
                pos = CompatibleTool.position(targetX + t.content.width, 0);
                break;
            case 3://单列VERTICAL（TOP_TO_BOTTOM）、网格HORIZONTAL（TOP_TO_BOTTOM）
                targetY = pos.top;
                if (offset != null)
                    ...
                    targetY += t._topGap;
                pos = CompatibleTool.position(0, -targetY);
                break;
            case 4://单列VERTICAL（BOTTOM_TO_TOP）、网格HORIZONTAL（BOTTOM_TO_TOP）
                ...
                pos = CompatibleTool.position(0, -targetY + t.content.height);
                break;
        }
